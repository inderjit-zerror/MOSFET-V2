...
export default function CementProcessSlider() {
  ...
  return (
    <section className="relative w-full px-5 py-10 sm:px-[10vw] sm:py-[10vh] BGRed ">
      ...
      <div className="grid grid-cols-1 md:h-[99svh] md:grid-cols-2">
        {/* Left: Image */}
        <div className="relative aspect-[4/3] bg-[#ECEEE9]/90 w-full overflow-hidden md:aspect-auto md:min-h-[650px]">
          <div
            ref={imageRef}
        ...
        <div className="relative flex min-h-[420px] flex-col items-center justify-center BGTint px-6 py-12 pb-24 sm:px-10 sm:py-16 sm:pb-16 text-center md:min-h-[650px]">
          <div
            ref={iconRef}
            className="mb-6 sm:mb-8 flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-xl border border-[#e8552f]/60"
          >
          ...
          <p
            ref={descRef}
            className="mx-auto paragraph mt-6 max-w-[95%] sm:max-w-[90%] text-[0.9rem] sm:text-[1rem] text-justify PH leading-[1.6] text-[#4a4a42]"
          >
            {slide.description}
          </p>

          {/* Progress bars */}
          <div className="mt-8 sm:mt-10 flex w-full max-w-md gap-3">
            {SLIDES.map((_, i) => (
              <div
                key={i}
                ref={(el) => (barsRef.current[i] = el)}
